...
func TestGetNestedValue(t *testing.T) {
	...
	Convey("Given a field and a BSON document, on calling getNestedValue", t,
		func() {
			Convey("the value of the key should be correct for unnested "+
				"documents", func() {
				bsonDocument := &bson.D{bson.DocElem{"a", 3}}
				So(getNestedValue("a", bsonDocument), ShouldEqual, 3)
			})
			Convey("the value of the key should be correct for nested "+
				"document fields", func() {
				bsonDocument := &bson.D{bson.DocElem{"a", bson.DocElem{"b", 4}}}
				So(getNestedValue("a.b", bsonDocument), ShouldEqual, 4)
			})
			Convey("the value of the key should be nil for unnested document "+
				"fields that do not exist", func() {
				bsonDocument := &bson.D{bson.DocElem{"a", 4}}
				So(getNestedValue("c", bsonDocument), ShouldBeNil)
			})
			Convey("the value of the key should be nil for nested document "+
				"fields that do not exist", func() {
				bsonDocument := &bson.D{
					bson.DocElem{"a", bson.DocElem{"b", 4}},
					bson.DocElem{"b", bson.DocElem{"c", 5}},
				}
				So(getNestedValue("a.c", bsonDocument), ShouldBeNil)
				So(getNestedValue("b.c", bsonDocument), ShouldEqual, 5)
				So(getNestedValue("a", bsonDocument), ShouldResemble, bson.DocElem{"b", 4})
			})
			Convey("the value of the key should be nil for nil document"+
				"values", func() {
				So(getNestedValue("a", &bson.D{bson.DocElem{"a", nil}}), ShouldBeNil)
			})
		})
}
...
func TestConstructUpsertDocument(t *testing.T) {
	...
	Convey("Given a set of upsert fields and a BSON document, on calling "+
		...
		Convey("the key/value combination in the upsert document should be "+
			...
			bsonDocument := &bson.D{bson.DocElem{"a", 3}, bson.DocElem{"b", "string value"}}
			upsertFields := []string{"a"}
		...
		Convey("the key/value combination in the upsert document should be "+
			...
			bsonDocument := &bson.D{
				bson.DocElem{"a", bson.DocElem{testCollection, 4}},
				bson.DocElem{"b", "string value"},
			}
			upsertFields := []string{"a.c"}
			expectedDocument := bson.D{bson.DocElem{"a.c", 4}}
			upsertDocument := constructUpsertDocument(upsertFields, bsonDocument)
			So(upsertDocument, ShouldResemble, expectedDocument)
		})
		...
		Convey("the upsert document should be nil if the fields are empty", func() {
			bsonDocument := &bson.D{bson.DocElem{"a", 3}, bson.DocElem{"b", "string value"}}
			upsertFields := []string{}
			upsertDocument := constructUpsertDocument(upsertFields, bsonDocument)
			So(upsertDocument, ShouldBeNil)
		})
		Convey("the upsert document should be nil if the document is nil", func() {
			upsertFields := []string{testCollection}
			upsertDocument := constructUpsertDocument(upsertFields, nil)
			So(upsertDocument, ShouldBeNil)
		})
	})
}
